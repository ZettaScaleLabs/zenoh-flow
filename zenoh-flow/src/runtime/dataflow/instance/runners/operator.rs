...
use crate::{Configuration, Inputs, NodeId, Outputs};
...
//     pub(crate) operator: Arc<dyn crate::traits::Operator>,
...
// #[async_trait]
// impl Runnable for OperatorRunner {
...
pub enum OperatorRunner {
    Ready(Operator<Ready>),
    Running(Operator<Running>),
}

pub struct Operator<State> {
    pub(crate) id: NodeId,
    pub(crate) context: Context,
    ...
    pub(crate) implementation: Arc<dyn crate::traits::Operator>,
    pub(crate) _library: Option<Arc<Library>>,
}
...
impl Operator<Ready> {
    pub fn new(context: Context, operator: OperatorLoaded, io: (Inputs, Outputs)) -> Self {
        let (inputs, outputs) = io;
        let OperatorLoaded {
            id,
            inputs: _,
    ...
    pub fn start(self) -> Operator<Running> {
        let Self {
            id,
            mut context,
            configuration,
        ...
        let c_outputs = outputs.clone();

        let iteration = async_std::task::block_on(async {
            implementation
                .setup(&mut context, &configuration, c_inputs, c_outputs)
                .await
        });
...
impl Operator<Running> {
    pub fn stop(self) -> Operator<Ready> {
        let Self {
            id,
            context,
            configuration,
